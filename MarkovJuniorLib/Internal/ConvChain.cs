...
namespace MarkovJuniorLib.Internal
{
    ...
    class ConvChainNode : Node
    {
        ...
        override protected bool Load(ModelConfig config, XElement xelem, bool[] symmetry, Grid grid)
        {
            ...
            temperature = xelem.Get("temperature", 1.0f);
            ...
            for (int k = 0; k < weights.Length; k++) if (weights[k] <= 0) weights[k] = 0.1f;
            return true;
        }
        ...
        override public bool Go()
        {
            ...
            for (int k = 0; k < state.Length; k++)
            {
                ...
                if (temperature != 1) q = Mathf.Pow(q, 1.0f / temperature);
                if (q > ip.random.NextFloat()) Toggle(state, r);
            }
