            ...
            if (potentials != null)
            {
                ...
                for (int m = 0; m < matchCount; m++)
                {
                    ...
                    if (heuristic != null)
                    {
                        ...
                        float u = ip.random.NextFloat();
                        list.Add((m, temperature > 0 ? Mathf.Pow(u, Mathf.Exp((h - firstHeuristic) / temperature)) : -h + 0.001f * u));
                    }
                }
                (int, float)[] ordered = list.OrderBy(pair => -pair.Item2).ToArray();
