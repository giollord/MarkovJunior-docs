    ...
    class PathNode : Node
    {
        ...
        public override bool Go()
        {
            ...
            foreach (var p in startPositions)
            {
                ...
                float noise = 0.1f * localRandom.NextFloat();
        ...
        (int, int, int) Direction(int x, int y, int z, int dx, int dy, int dz, int[] generations, Random random)
        {
            ...
            else
            {
                ...
                if (inertia && (dx != 0 || dy != 0 || dz != 0))
                {
                    ...
                    {
                        float noise = 0.1f * random.NextFloat();
                        float cos = (c.x * dx + c.y * dy + c.z * dz) / Mathf.Sqrt((c.x * c.x + c.y * c.y + c.z * c.z) * (dx * dx + dy * dy + dz * dz));
