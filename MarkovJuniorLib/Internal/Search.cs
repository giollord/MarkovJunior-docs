...
namespace MarkovJuniorLib.Internal
{
    ...
    static class Search
    {
        ...
        public static byte[][] Run(byte[] present, int[] future, Rule[] rules, int MX, int MY, int MZ, int C, bool all, int limit, float depthCoefficient, int seed)
        {
            ...
            {
                Debug.LogError("INCORRECT PROBLEM");
                return null;
            }
    ...
    class Board
    {
        ...
        /// <param name="depthCoefficient"><inheritdoc cref="RuleNode.depthCoefficient" path="/summary"/></param>
        public float Rank(Random random, float depthCoefficient)
        {
            ...
            float result = depthCoefficient < 0.0f ? 1000 - depth : forwardEstimate + backwardEstimate + 2.0f * depthCoefficient * depth;
            return result + 0.0001f * random.NextFloat();
        }
