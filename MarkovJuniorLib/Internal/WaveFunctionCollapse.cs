...
namespace MarkovJuniorLib.Internal
{
    ...
    abstract class WFCNode : Branch
    {
        ...
        override protected bool Load(ModelConfigBase config, NodeInfo parentNodeInfo, XElement xelem, bool[] parentSymmetry, Grid grid)
        {
            ...
            if (shannon)
            {
                weightLogWeights = new float[P];

                for (int t = 0; t < P; t++)
                {
                    weightLogWeights[t] = weights[t] * Mathf.Log(weights[t]);
            ...
            distribution = new float[P];
            return base.Load(config, parentNodeInfo, xelem, parentSymmetry, newgrid);
        }
            ...
            for (int z = 0; z < MZ; z++) for (int y = 0; y < MY; y++) for (int x = 0; x < MX; x++)
                    {
                        ...
                        float entropy = shannon ? wave.entropies[i] : remainingValues;
                        if (remainingValues > 1 && entropy <= min)
                        {
                            // add a small noise term to randomly choose between equal minima
                            float noise = 1E-6f * random.NextFloat();
        ...
        void Observe(int node, Random random)
        {
            ...
            for (int t = 0; t < P; t++) distribution[t] = w[t] ? weights[t] : 0.0f;
            int r = distribution.Random(random.NextFloat());
        ...
        void Ban(int i, int t)
        {
            ...
            if (shannon)
            {
                float sum = wave.sumsOfWeights[i];
                wave.entropies[i] += wave.sumsOfWeightLogWeights[i] / sum - Mathf.Log(sum);
    ...
    class Wave
    {
        ...
        public float[] sumsOfWeightLogWeights;
        ...
        public Wave(int length, int P, int D, bool shannon)
        {
            ...
            if (shannon)
            {
                sumsOfWeights = new float[length];
                sumsOfWeightLogWeights = new float[length];
                entropies = new float[length];
            }
        }
        ...
        /// <param name="shannon"><inheritdoc cref="WFCNode.shannon" path="/summary"/></param>
        public void Init(int[][][] propagator, float sumOfWeights, float sumOfWeightLogWeights, float startingEntropy, bool shannon)
        {
            int P = data[0].Length;
